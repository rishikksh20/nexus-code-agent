...
class PersistenceManager:
    ...
    def _write_json_atomic(self, file_path: Path, payload: dict[str, Any]) -> None:
    ...
    def _read_snapshot_file(self, file_path: Path) -> SessionSnapshot | None:
        try:
    ...
    def load_session(self, session_id: str) -> SessionSnapshot | None:
        ...
        return self._read_snapshot_file(file_path)

    def list_sessions(self) -> list[dict[str, Any]]:
        sessions = []
        for file_path in self.sessions_dir.glob("*.json"):
            snapshot = self._read_snapshot_file(file_path)
            if snapshot is None:
                continue
            sessions.append(
                {
