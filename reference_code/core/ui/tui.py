...
from core.utils.paths import display_path_rel_to_cwd
...
class TUI:
    ...
    def _extract_read_file_code(self, text: str) -> tuple[int, str] | None:
        "Showing line x-y of z\n\n1|def main():\n 2|"
        body = text
        header_match = re.match(r"^Showing line (\d+)-(\d+) of (\d+)\n\n", text)
        if header_match:
        ...
        for line in body.splitlines():
            ...
            if start_line is None or line_no < start_line:
                start_line = line_no
    ...
    def tool_call_complete(
        ...
        if not blocks:
            blocks.append(Text("(no tool output)", style="muted"))
