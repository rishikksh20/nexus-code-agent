...
def _subagent_guidance_lines(records) -> list[str]:
    ...
    lines = [
        ...
        "- For simple implementation tasks with an obvious target file, missing file, or direct port from a known source file, skip `subagent_explorer`; call `subagent_coding` once with the target path, source path, style reference, and a small pre-edit tool budget.",
