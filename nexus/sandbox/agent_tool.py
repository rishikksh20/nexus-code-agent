...
def _direct_subagent_system_prompt(
    ...
    *,
    ...
    return (
        ...
        "For implementation tasks, status `completed` requires at least one real file change; otherwise use `blocked`, "
        "`needs_clarification`, or `failed`.\n\n"
        "Return only a JSON object. Set status to one of: completed, blocked, needs_clarification, failed, approved, "
        "issues_found, or failed_verification. Common keys: status, summary, findings, changed_files, "
