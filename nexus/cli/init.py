...
def _local_config_toml(*, workspace_root: Path, project_name: str, project_description: str) -> str:
    return "\n".join(
        [
            ...
            '#   insert_edit_into_file, apply_patch, modify_file, glob, grep,',
            '#   list_dir, bash, memory, todos, web_fetch, web_search',
            '# Add external tool names here when enabling plugins, MCP, or the sandboxed command tool.',
            'allowed_tools = ["get_time", "read_file", "write_file", "edit", "insert_edit_into_file", "apply_patch", "modify_file", "glob", "grep", "list_dir", "bash", "memory", "todos", "web_fetch", "web_search"]',
            'denied_tools = []',
