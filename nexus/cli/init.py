...
def _local_config_toml(*, workspace_root: Path, project_name: str, project_description: str) -> str:
    return "\n".join(
        [
            ...
            '#   { name = "explorer", description = "Summarize a bounded codebase slice.", goal_prompt = "Read-only explorer. Stop once you have enough evidence to answer.", allowed_tools = ["read_file", "glob", "grep", "list_dir", "lsp"] },',
