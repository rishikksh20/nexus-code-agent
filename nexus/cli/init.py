...
def _local_config_toml(*, workspace_root: Path, project_name: str, project_description: str) -> str:
    return "\n".join(
        [
            ...
            '# Optional specialists for multi-agent mode:',
            '# delegation_subagents = [',
            '#   { name = "research", description = "Investigate repo structure and summarize findings.", goal_prompt = "Read-only research agent. Do not modify files.", allowed_tools = ["read_file", "glob", "grep", "list_dir", "lsp"] },',
